defmodule Acupuncture.Plugin do
  ...
  registration:
    ...
    * `priv/repo/acupuncture_field_definitions.exs` /
      `priv/repo/acupuncture_import.exs` (under `kiln/priv/`) — the one-time
      Sanity migration: custom-field definitions, then the content import
      (plain scripts run with `mix run`; they need no plugin callbacks).

  Activate in a deployment's config (see `kiln/config/project.exs`):
